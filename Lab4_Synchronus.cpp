...
int main(int argc, char** argv){
    ...
    MPI_Bcast(&heat_left, 1, MPI_INT, 1, MPI_COMM_WORLD);
    MPI_Bcast(&heat_right, 1, MPI_INT, 1, MPI_COMM_WORLD);
    MPI_Bcast(&heat_top, 1, MPI_INT, 1, MPI_COMM_WORLD);
    MPI_Bcast(&heat_bottom, 1, MPI_INT, 1, MPI_COMM_WORLD);
    MPI_Bcast(&radi_left, 1, MPI_INT, 2, MPI_COMM_WORLD);
    MPI_Bcast(&radi_right, 1, MPI_INT, 2, MPI_COMM_WORLD);
    MPI_Bcast(&radi_top, 1, MPI_INT, 2, MPI_COMM_WORLD);
    MPI_Bcast(&radi_bottom, 1, MPI_INT, 2, MPI_COMM_WORLD);
    ...
    while(!done){
        if(world_rank==0){
            ...
            if(world_rank%4==1 && world_rank<world_size-2){
                MPI_Send(right_snd,y_size,MPI_DOUBLE,world_rank+2,0,MPI_COMM_WORLD);
            }else if(world_rank%4==3){
                MPI_Recv(left_rcv,y_size,MPI_DOUBLE,world_rank-2,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            }
            // Step 2 : from 4n+1 to 4n+3 (left_snd)
            if(world_rank%4==1 && world_rank>1){
                MPI_Send(left_snd,y_size,MPI_DOUBLE,world_rank-2,0,MPI_COMM_WORLD);
            }else if(world_rank%4==3 && world_rank<world_size-2){
                MPI_Recv(right_rcv,y_size,MPI_DOUBLE,world_rank+2,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            }
